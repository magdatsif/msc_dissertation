...
# Determines the y position of the object in the 5x1 grid
...
# Determines the x position of the object in the 5x1 grid
def quantize_x_axis(object):
    bin_x = 0
    if 0 <= object.posx + object.image_width/2 <= window_width/5:
        bin_x = 0 
    elif window_width/5 < object.posx + object.image_width/2 <= 2*window_width/5:
        bin_x = 1
    elif 2*window_width/5 < object.posx + object.image_width/2 <= 3*window_width/5:
        bin_x = 2
    elif 3*window_width/5 < object.posx + object.image_width/2 <= 4*window_width/5:
        bin_x = 3 
    elif 4*window_width/5 < object.posx + object.image_width/2 <= window_width:
        bin_x = 4 
...
class Satellite:
    def __init__(self, posx, posy):
        ...
        self.image = pygame.transform.scale(self.image, (int(self.image_width/1.1), int(self.image_height/1.1)))
        self.image_width, self.image_height = self.image.get_rect().size
    ...
    # Returns satellite's y position in the 5x5 grid
...
class Debris:
    def __init__(self, posx):
        ...
        self.posx = posx
        self.image_width, self.image_height = self.image.get_rect().size
        self.image = pygame.transform.scale(self.image, (int(self.image_width/1.35), int(self.image_height/1.35)))
        self.image_width, self.image_height = self.image.get_rect().size
        
        # Position of obstacle either up or down in the window
        self.rand = random.randint(0,2)
        if self.rand:
            self.posy = random.randint(0,70)
        else:
            self.posy = random.randint(window_height- self.image_height -70, window_height - self.image_height)
    ...
    # Returns satellite's position in the 5x5 grid
    def position_in_quantised_space(self):
        return quantize_y_axis(self), quantize_x_axis(self)
...
class PyGameObjectAvoidance:
    def __init__(self):
        pygame.init()
        pygame.display.set_caption('DebrisAvoidance-v0')
